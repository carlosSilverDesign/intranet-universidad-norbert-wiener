document.addEventListener('DOMContentLoaded', () => {
  ...
  // 6. Resaltado interactivo de los enlaces del Navbar y soporte de acordeón
  const menuLinks = document.querySelectorAll('.sidebar-menu-link');
  menuLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      const parentLi = link.closest('.sidebar-menu-item, .sidebar-nested-item');
      const hasSubmenu = parentLi && (parentLi.querySelector('.sidebar-submenu') || parentLi.querySelector('.sidebar-nested-menu'));

      // Si el enlace tiene un submenú hijo directo, alternar apertura/cierre
      if (hasSubmenu && link.parentElement === parentLi) {
        e.preventDefault();
      ...
      const textElem = link.querySelector('.text');
      if (textElem) {
        console.log(`Navegando a la sección: ${textElem.textContent.trim()}`);
      }
